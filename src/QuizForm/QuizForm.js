...
class QuizForm extends React.Component {
  ...
  onAddOption = (index) => {
    this.setState(
      produce(this.state, (draft) => {
        draft.questions[index].options.push({optionValue: "", optionImage: ""});
      })
    );
  };
  ...
  questionImageToBase64 = async (item) => {
    ...
    const newOptions = await Promise.all(question.options.map((option) => {
  ...
  render() {
    return (
      <div className="quizContent">
                    ...
                    <React.Fragment>
                      <div className="col-md-12">
                        ...
                        >
                         ...
                         <input
                  type="file"
                  onChange={(e) => this.onOptionImageUpload(e, index,optionIndex)}
                />
                      </div>
                    </React.Fragment>
                  );
        ...
        <div class="col-md-12">
          <button id="createQuiz" onClick={this.onSaveClick}>Save Questions</button>
        </div>
      </div>
    );
  }
